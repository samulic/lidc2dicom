...
class LIDC2DICOMConverter:
    ...
    def convertSingleAnnotation(self, nCount, aCount, a, ctDCM, noduleUID, volume, seriesDir):
        ...
        if not os.path.exists(nrrdSegFile):
            self.logger.error(
                "Cannot convert single annotation - file does not exist")
            raise Exception(
                "Cannot convert single annotation - file does not exist")
        ...
        converterCmd = ['itkimage2segimage', "--inputImageList", nrrdSegFile, "--inputDICOMDirectory",
                        seriesDir, "--inputMetadata", jsonSegFile, "--outputDICOM", dcmSegFile]
        if self.args.skip:
            converterCmd.append('--skip')
        self.logger.info("Converting to DICOM SEG with "+str(converterCmd))

        sp = subprocess.Popen(
            converterCmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        volumeItem["units"] = {
            "CodeValue": "mm3", "CodingSchemeDesignator": "UCUM", "CodeMeaning": "cubic millimeter"}
        volumeItem["measurementModifier"] = {"CodeValue": "122503", "CodingSchemeDesignator": "DCM",
                                             "CodeMeaning": "Integration of sum of closed areas on contiguous slices"}
        volumeItem["measurementAlgorithmIdentification"] = {
            "AlgorithmName": "pylidc", "AlgorithmVersion": "0.2.0"}

        # CID 7470
        diameterItem = {}
        diameterItem["value"] = '%E' % Decimal(a.diameter)
        diameterItem["quantity"] = {
            "CodeValue": "M-02550", "CodingSchemeDesignator": "SRT", "CodeMeaning": "Diameter"}
        diameterItem["units"] = {
            "CodeValue": "mm", "CodingSchemeDesignator": "UCUM", "CodeMeaning": "millimeter"}
        diameterItem["measurementAlgorithmIdentification"] = {
            "AlgorithmName": "pylidc", "AlgorithmVersion": "0.2.0"}
        ...
        surfaceItem["quantity"] = {
            "CodeValue": "C0JK", "CodingSchemeDesignator": "IBSI", "CodeMeaning": "Surface area of mesh"}
        surfaceItem["units"] = {
            "CodeValue": "mm2", "CodingSchemeDesignator": "UCUM", "CodeMeaning": "square millimeter"}
        surfaceItem["measurementAlgorithmIdentification"] = {
            "AlgorithmName": "pylidc", "AlgorithmVersion": "0.2.0"}
        ...
        for attribute in self.conceptsDictionary.keys():
            ...
            try:
                ...
                qItem["conceptValue"] = self.valuesDictionary[attribute][str(
                    getattr(a, attribute))]
                qualitativeEvaluations.append(qItem)
            except KeyError:
                self.logger.info("Skipping invalid attribute: " +
                                 attribute+': '+str(getattr(a, attribute)))
                continue
        ...
        srName = "Nodule "+str(nCount+1) + " - Annotation " + \
            a._nodule_id + " measurements"
        ...
        converterCmd = ['tid1500writer', "--inputMetadata", jsonSRFile, "--inputImageLibraryDirectory",
                        seriesDir, "--inputCompositeContextDirectory", self.tempSubjectDir, "--outputDICOM", dcmSRFile]
    ...
    def convertForSubject(self, subjectID):
        ...
        for scan in scans:
            ...
            self.tempSubjectDir = os.path.join(
                self.tempDir, s, studyUID, seriesUID)
            # Avoid re-running conversion if files already exist
            converted_dcmFiles = glob.glob(
                os.path.join(self.tempSubjectDir, "*.dcm"))
            converted_nrrdFiles = glob.glob(
                os.path.join(self.tempSubjectDir, "*.nrrd"))
            # We have one nrrd more than dicoms because of the complete scan (i.e. '*_CT.nrrd' file)
            if len(converted_dcmFiles) != 0 and len(converted_dcmFiles) == (len(converted_nrrdFiles) - 1):
                self.logger.info(
            ...
            if not ok:
                self.logger.warning(
                    "Geometry inconsistent for subject %s" % (s))

            scanNRRDFile = os.path.join(self.tempSubjectDir, s+'_CT.nrrd')
            if not os.path.exists(scanNRRDFile):
                # convert
                # tempDir = tempfile.mkdtemp()
                plastimatchCmd = ['plastimatch', 'convert',
                                  '--input', seriesDir, '--output-img', scanNRRDFile]
                self.logger.info(
                    "Running plastimatch with "+str(plastimatchCmd))

                sp = subprocess.Popen(
                    plastimatchCmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
                (stdout, stderr) = sp.communicate()
                self.logger.info("plastimatch stdout: "+stdout.decode('ascii'))
                self.logger.warning(
                    "plastimatch stderr: "+stderr.decode('ascii'))

                self.logger.info('plastimatch completed')
                self.logger.info(
                    "Conversion of CT volume OK - result in "+scanNRRDFile)
            else:
                self.logger.info(
                    scanNRRDFile+" exists. Not rerunning volume reconstruction.")
            ...
            self.logger.info(
                "Have %d annotations for subject %s" % (len(anns), s))
            ...
            for nCount, nodule in enumerate(scan.cluster_annotations()):
                ...
                # by default, pydicom uses 2.25 root
                ...
                for aCount, a in enumerate(nodule):
                    ...
                    annotationFileName = "Nodule " + \
                        str(nCount+1) + " - Annotation " + a._nodule_id+'.nrrd'
                    self.saveAnnotationAsNRRD(a, volume, os.path.join(
                        self.tempSubjectDir, annotationFileName))

                    self.convertSingleAnnotation(
                        nCount, aCount, a, ctDCM, noduleUID, volume, seriesDir)
            ...
            for ua in anns:
                if ua.id not in clusteredAnnotationIDs:
                    ...
                    noduleUID = pydicom.uid.generate_uid(prefix=None)
                    self.convertSingleAnnotation(
                        nCount, aCount, ua, ctDCM, noduleUID, volume, seriesDir)
    ...
    def makeCompositeObjects(self, subjectID):
        ...
        for scan in scans:
            ...
            subjectScanTempDir = os.path.join(
                self.tempDir, s, studyUID, seriesUID)
            allSegmentations = glob.glob(os.path.join(
                subjectScanTempDir, 'Nodule*Annotation*.nrrd'))
            ...
            for segID, seg in enumerate(allSegmentations):
                ...
                prefix = seg[:-5]
                matches = re.match(
                    'Nodule (\d+) - Annotation (.+)\.', os.path.split(seg)[1])
                print("Nodule: "+matches.group(1) +
                      " Annotation: "+matches.group(2))
            ...
            segMetadata["SeriesNumber"] = str(
                int(ctDCM.SeriesNumber if ctDCM.SeriesNumber else 0)+self.instanceCount)
            self.instanceCount = self.instanceCount+1

            # run SEG converter

            allSegsJSON = os.path.join(
                subjectScanTempDir, "all_segmentations.json")
            with open(allSegsJSON, "w") as f:
                json.dump(segMetadata, f, indent=2)

            compositeSEGFileName = os.path.join(
                subjectScanTempDir, "all_segmentations.dcm")
            nrrdSegFileList = nrrdSegFileList[:-1]

            converterCmd = ['itkimage2segimage', "--inputImageList", nrrdSegFileList, "--inputDICOMDirectory",
                            seriesDir, "--inputMetadata", allSegsJSON, "--outputDICOM", compositeSEGFileName]
            if self.args.skip:
                converterCmd.append('--skip')
            self.logger.info("Converting to DICOM SEG with "+str(converterCmd))
            ...
            self.logger.info("itkimage2segimage stdout: " +
                             stdout.decode('ascii'))
            self.logger.warning(
                "itkimage2segimage stderr: "+stderr.decode('ascii'))
            ...
            allSrsJSON = os.path.join(
                subjectScanTempDir, "all_measurements.json")
            with open(allSrsJSON, "w") as f:
                json.dump(srMetadata, f, indent=2)

            compositeSRFileName = os.path.join(
                subjectScanTempDir, "all_measurements.dcm")
            nrrdSegFileList = nrrdSegFileList[:-1]

            converterCmd = ['tid1500writer', "--inputMetadata", allSrsJSON, "--inputImageLibraryDirectory",
                            seriesDir, "--inputCompositeContextDirectory", subjectScanTempDir, "--outputDICOM", compositeSRFileName]
...
def main():
    ...
    if args.subjectIDs:
        ...
        logger.info("Processing subjects from " +
                    str(args.subjectRange[0])+" to "+str(args.subjectRange[1])+" inclusive")
